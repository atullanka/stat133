...
name <- "Atul Lanka"
github.name <- "atullanka"
email.address <- "atul.lanka@berkeley.edu"
...
set.seed(42)
z <- sort(rnorm(111,0,1),decreasing = F)
...
# a random permutation of the even numbers from 2 to 222
  set.seed(31415)
v <- sample(seq(2,222,by =2),size = 111, replace = F )
...
set.seed(2718)
w <- sample(rep(1:13,4), size = 52, replace = F)
...
# Each vector of length 100 of Poisson (hint:rpois) random variables with mean 5
  set.seed(71)
l <- replicate(12, rpois(100, 5), simplify = F)
...
first.time <- infants[infants$marital == "Married" & infants$parity ==1,]
ed<- first.time$ed
t <-table(ed)

# [2 pt]
# Calculate [mw], the average birthweight ($bwt) of all babies whose were full term, i.e. gestation equal or more than 259 days.
mw <- mean(as.vector(infants$bwt[infants$gestation >= 259]),na.rm = T)
...
color <- rainbow(3)
species.col <- color[iris$Species]
plot(iris$Petal.Length, iris$Petal.Width, type = 'p', xlab ="Petal Length",
     ylab = "Petal Width", col = species.col)

# [3 pt]
# Make a scatterplot of ( sepal length / petal length) as a function of index (order)
# Color the plotting symbol by Species (any 3 colors)
$%#^$^#^#%#%#$%#$%3
plot(iris$Sepal.Length/iris$Petal.Length, y = c(1:nrow(iris)), type = 'p',
     ylab =" Sepal Length / Petal Length",
     xlab = "Index", col = species.col)
...
# Add x-axis labels
hist(infants$age, xlab = "Mother's age")
with(infants$age, plot(density(infants$age, na.rm= T), col = "blue", 
                       type = 'l' ))
...
mean.long.cache <- mean.cache[length(Cache500[[1]]) > 50]
...
for(x in 1:length(phrases)){
  multiple.al[[x]] = length(gregexpr('at', phrases)[[x]])
  }
text3 <- phrases[unlist(multiple.al) > 1] 

# [3 pts]
# Create a vector [tests] that is of length 200 and has the entries
# "test1", "test2", ..., "test200"
tests <- vector(length = 200)
for(i in 1:200){
  tests[i] = paste0("test",i)
}
# [3 pts]
